 ...
 Scenario: Test invalid layer
  ...
  Scenario: Test preview FP and preview cloud FP with legacy app.
    Given s2i build https://github.com/wildfly/wildfly-s2i from test/test-app with env and True using legacy-s2i-images
      | variable                             | value         |
      | GALLEON_PROVISION_LAYERS | cloud-server |
      | GALLEON_PROVISION_FEATURE_PACKS | org.wildfly:wildfly-preview-feature-pack:27.0.0.Final, org.wildfly.cloud:wildfly-preview-cloud-galleon-pack:2.0.0.Final |
      | MAVEN_REPO_ID | opensaml |
      | MAVEN_REPO_NAME | opensaml |
      | MAVEN_REPO_URL | https://build.shibboleth.net/nexus/content/groups/public |
   Then container log should contain WFLYSRV0025
