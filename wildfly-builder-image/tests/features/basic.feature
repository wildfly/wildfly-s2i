...
Feature: Wildfly basic tests
  ...
  Scenario: Check that the legacy default config provisioned using galleon plugin works fine
   Given s2i build http://github.com/wildfly/wildfly-s2i from test/test-app-default-config with env and True
   | variable                 | value           |
  ...
  Scenario: Check if image version and release is printed on boot
   Given s2i build http://github.com/wildfly/wildfly-s2i from test/test-app with env and True
   | variable                 | value           |
   ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
   Then container log should contain Running wildfly/wildfly-s2i-jdk
   Then container log should contain WFLYSRV0025
