...
Feature: OIDC tests
  ...
  Scenario: Check oidc subsystem configuration, legacy.
     Given XML namespaces
       | prefix | url                          |
       | ns     | urn:wildfly:elytron-oidc-client:1.0 |
     Given s2i build git://github.com/jfdenise/wildfly-s2i from test/test-app-elytron-oidc-client-legacy with env and True using keycloak-v2-and-oidc-support
       | variable               | value                                            |
       | OIDC_PROVIDER_NAME | keycloak |
       | OIDC_PROVIDER_URL           | http://localhost:8080/auth/realms/demo    |
       | OIDC_SECURE_DEPLOYMENT_ENABLE_CORS        | true                          |
       ...
       | GALLEON_PROVISION_FEATURE_PACKS|org.wildfly:wildfly-galleon-pack:25.0.1.Final,org.wildfly.cloud:wildfly-cloud-galleon-pack:1.0.0.Alpha5 |
