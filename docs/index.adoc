...
* JDK25 based runtime image: `docker pull quay.io/wildfly/wildfly-runtime:latest-jdk25`, to run Jakarta EE 11 server

* JDK21 based runtime image: `docker pull quay.io/wildfly/wildfly-runtime:latest-jdk21`, to run Jakarta EE 10+ server
...
When running a WildFly server inside the WildFly S2i runtime, you can use these link:https://github.com/rh-openjdk/redhat-openjdk-containers/blob/ubi10/modules/jvm/module.yaml[environment variables] to configure the Java VM.
...
== WildFly - UBI10 S2I builder image
...
* JDK25 based builder image: `docker pull quay.io/wildfly/wildfly-s2i:latest-jdk25`, to run Jakarta EE 11 server

* JDK21 based builder image: `docker pull quay.io/wildfly/wildfly-s2i:latest-jdk21`, to run Jakarta EE 10+ server
...
The WildFly S2i builder image is exposing a set of link:https://github.com/rh-openjdk/redhat-openjdk-containers/blob/ubi10/modules/s2i/core/module.yaml[environment variables] to fine tune the server execution.
...
For example, usage of JDK 21 builder and runtime imagestream tags:

[source,xml]
----
  s2i:
    builderImage: wildfly-s2i:latest-jdk21
    runtimeImage: wildfly-runtime:latest-jdk21
    kind: ImageStreamTag
----
